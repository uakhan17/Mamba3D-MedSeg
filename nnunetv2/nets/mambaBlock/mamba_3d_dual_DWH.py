...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.conv2d = nn.Conv2d(in_channels=config.d_inner, out_channels=config.d_inner, 
    ...
    def forward(self, x, size_dhw):
        ...
        xz = xz.reshape(xz.shape[0], size_dhw[0], size_dhw[1], size_dhw[2], -1) # (B, D, H, W, 2*ED)

        x, z = xz.chunk(2, dim=-1) # (B, D, H, W, ED), (B, D, H, W, ED)
        z = z.reshape(z.shape[0] * size_dhw[0], size_dhw[1], size_dhw[2], -1).permute(0, 3, 1, 2).contiguous().flatten(2).transpose(1, 2) # (B*D, H*W, ED)

        # x branch
        x = x.reshape(x.shape[0] * size_dhw[0], size_dhw[1], size_dhw[2], -1).permute(0, 3, 1, 2).contiguous() # (B*D, ED, H, W)
        x = self.conv2d(x) # (B*D, ED, H, W)
        x = x.flatten(2).transpose(1, 2) # (B*D, H*W, ED)
        
        x = F.silu(x)
        y = self.ssm(x, z) # (B*D, H*W, ED)

        # print(y.shape)

        y = y.reshape(xz.shape[0], size_dhw[0]*size_dhw[1]*size_dhw[2], -1) # (B, D*H*W, ED)


        if self.config.bidirectional:

            x_b, z_b = xz.chunk(2, dim=-1) # (B, D, H, W, ED), (B, D, H, W, ED)
            z_b = z_b.permute(0, 2, 3, 1, 4).reshape(z_b.shape[0]*size_dhw[1]*size_dhw[2], size_dhw[0], -1).contiguous() # (B*H*W, D, ED)

            x_b = x_b.permute(0, 2, 3, 1, 4).reshape(x_b.shape[0]*size_dhw[1]*size_dhw[2], size_dhw[0], -1).contiguous().transpose(1, 2) # (B*H*W, ED, D)
            x_b = self.conv1d_b(x_b) # (B*H*W, ED, D)
            x_b = x_b.transpose(1, 2) # (B*H*W, D, ED)

            x_b = F.silu(x_b)
            y_b = self.ssm(x_b, z_b) # (B*H*W, D, ED)

            # print(y_b.shape)

            y_b = y_b.reshape(xz.shape[0], size_dhw[0]*size_dhw[1]*size_dhw[2], -1) # (B, D*H*W, ED)
        ...
        if self.config.use_cuda:
            ...
            if not self.config.bidirectional:
                ...
                if self.config.divide_output:
                    return self.out_proj((y + y_b) / 2)
                else:
                    return self.out_proj(y + y_b)
