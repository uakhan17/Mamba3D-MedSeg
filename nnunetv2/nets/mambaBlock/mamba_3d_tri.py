...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.conv3d = nn.Conv3d(in_channels=config.d_inner, out_channels=config.d_inner, 
        ...
        if config.tridirectional:
            ...
            self.conv3d_b = nn.Conv3d(in_channels=config.d_inner, out_channels=config.d_inner, 
                                                kernel_size=config.d_conv, bias=config.conv_bias, 
                                                groups=config.d_inner,
                                                padding=(config.d_conv - 1)//2,
                                                stride=1)
            ...
            self.conv3d_c = nn.Conv3d(in_channels=config.d_inner, out_channels=config.d_inner, 
                                                kernel_size=config.d_conv, bias=config.conv_bias, 
                                                groups=config.d_inner,
                                                padding=(config.d_conv - 1)//2,
                                                stride=1)
    ...
    def forward(self, x, size_dhw):
        ...
        # x branch
        x = x.reshape(x.shape[0], size_dhw[0], size_dhw[1], size_dhw[2], -1).permute(0, 4, 1, 2, 3).contiguous() # (B, ED, D, H, W)
        x = self.conv3d(x) # (B, ED, D, H, W)
        x = x.flatten(2).transpose(1, 2) # (B, L, ED)
        ...
        if self.config.tridirectional:
            ...
            x_b = self.conv3d_b(x_b) # (B, ED, D, W, H)
            x_b = x_b.flatten(2).transpose(1, 2) # (B, L, ED)
            x_b = F.silu(x_b)
            y_b = self.ssm(x_b, z_b) # (B, L, ED)
            
            y_b = y_b.reshape(y_b.shape[0], size_dhw[0], size_dhw[2], size_dhw[1], -1) # (B, D, W, H, ED)
            
            y_b = y_b.permute(0, 4, 1, 3, 2).contiguous().flatten(2).transpose(1, 2)
            ...
            x_c = self.conv3d_c(x_c) # (B, ED, H, W, D)
            x_c = x_c.flatten(2).transpose(1, 2) # (B, L, ED)
            x_c = F.silu(x_c)
            y_c = self.ssm(x_c, z_c) # (B, L, ED)
            
            y_c = y_c.reshape(y_c.shape[0], size_dhw[1], size_dhw[2], size_dhw[0], -1) # (B, H, W, D, ED)
            
            y_c = y_c.permute(0, 4, 3, 1, 2).contiguous().flatten(2).transpose(1, 2)
