...
class MambaBlock(nn.Module):
    ...
    def forward(self, x, size_dhw):
        ...
        if self.config.bidirectional:
            ...
            # print(xz_r.shape)
            xz_b = torch.flip(torch.flip(torch.flip(xz_r, [1]), [2]), [3])
            # print(xz_b.shape)       
            xz_b = xz_b.reshape(xz_b.shape[0], size_dhw[0]*size_dhw[1]*size_dhw[2], -1).contiguous()
            
            x_b, z_b = xz_b.chunk(2, dim=-1) # (B, L, ED), (B, L, ED)
            x_b = x_b.reshape(x_b.shape[0], size_dhw[0], size_dhw[1], size_dhw[2], -1).permute(0, 4, 1, 2, 3).contiguous() # (B, ED, D, H, W)
            x_b = self.conv3d_b(x_b) # (B, ED, D, H, W)
            x_b = x_b.flatten(2).transpose(1, 2) # (B, L, ED)
            x_b = F.silu(x_b)
            y_b = self.ssm(x_b, z_b) # (B, L, ED)
            
            y_b = y_b.reshape(y_b.shape[0], size_dhw[0], size_dhw[1], size_dhw[2], -1) # (B, D, H, W, ED)
            y_b = torch.flip(torch.flip(torch.flip(y_b, [1]), [2]), [3])
            y_b = y_b.permute(0, 4, 1, 2, 3).contiguous().flatten(2).transpose(1, 2)
        ...
        #     output_b = y_b * z_b
        #     if self.config.divide_output:
        #         return self.out_proj((output + output_b.flip([1])) / 2)
        #     else:
        #         return self.out_proj(output + output_b.flip([1]))
