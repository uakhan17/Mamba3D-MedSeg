...
class MambaBlock(nn.Module):
    ...
    def forward(self, x, size_dhw):
        ...
        if self.config.bidirectional:
            x_b, z_b = xz.chunk(2, dim=-1) # (B, L, ED), (B, L, ED)
            ...
            perm_indices = torch.randperm(xz.size(1))
            x_b = x_b[:, perm_indices, :]
            z_b = z_b[:, perm_indices, :]

            x_b = F.silu(x_b)
            y_b = self.ssm(x_b, z_b)

            reverse_indices = torch.zeros_like(perm_indices)
            reverse_indices[perm_indices] = torch.arange(xz.size(1))
            y_b = y_b[:, reverse_indices, :]
