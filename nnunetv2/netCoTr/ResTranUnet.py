...
from nnunetv2.netCoTr import CNNBackbone
from nnunetv2.netCoTr.DeTrans.DeformableTrans import DeformableTransformer
from nnunetv2.netCoTr.DeTrans.position_encoding import build_position_encoding
...
class CoTr_decoder(nn.Module):
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', num_classes=None, weight_std=False, deep_supervision=False):
        super(CoTr_decoder, self).__init__()
    ...
    def forward(self, x_convs):
        ...
        if not self.deep_supervision:
            ...
            r = [result, ds0, ds1, ds2]
        return r


class U_ResTran3D(nn.Module):
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', img_size=None, num_classes=None, weight_std=False, deep_supervision=False, in_channels=1):
        ...
        self.decoder = CoTr_decoder(norm_cfg=norm_cfg, activation_cfg=activation_cfg, num_classes=num_classes, weight_std=weight_std, deep_supervision=deep_supervision)

    def forward(self, inputs):
        # # %%%%%%%%%%%%% CoTr
        x_convs = self.backbone(inputs)
        results = self.decoder(x_convs)
...
class ResTranUnet(nn.Module):
    ...
    ResTran-3D Unet
    """
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', in_channels=1, img_size=None, num_classes=None, weight_std=False, deep_supervision=True):
        super().__init__()
        self.U_ResTran3D = U_ResTran3D(norm_cfg, activation_cfg, img_size, num_classes, weight_std, deep_supervision, in_channels) # U_ResTran3D
